...
class TestPointEnumerator : public PointEnumerator {
 public:
  ...
  size_t GetCount() const override { return points_.size(); }

  bool EnumNext(GraphPoint& value) override {
    if (points_.empty()) {
      return false;
    }
...
class VirtualPointEnumerator : public PointEnumerator {
 public:
  ...
  size_t GetCount() const override { return last_index_ - index_ + 1; }
...
class VirtualDataSource : public GraphDataSource {
 public:
  ...
  double GetCurrentValue() const override { return dataset_.current(); }
